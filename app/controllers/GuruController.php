...
class GuruController extends SecureController{
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("nip","nama","jk","email","username","password","photo","user_role_id");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("kodeguru","nip","nama","jk","username","photo","user_role_id");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'nama' => 'required',
			...
			if(isset($modeldata['username'])){
				$db->where("username", $modeldata['username'])->where("kodeguru", $rec_id, "!=");
				if($db->has($tablename)){
					$this->view->page_error[] = $modeldata['username']." Already exist!";
				}
			} 
